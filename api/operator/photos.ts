...
export default async function handler(req: Request) {
  ...
  if (type.includes('multipart/form-data')) {
    ...
    if (!(file instanceof File) || !/^[a-f0-9-]+\/(before|during|after|issue)\/[A-Za-z0-9_.-]+$/.test(path)) return secureJson({ error: 'invalid upload' }, { status: 400 });
  ...
  if (!Array.isArray(body.paths) || body.paths.length > 50 || body.paths.some((path) => !/^[A-Za-z0-9_./-]+$/.test(path))) return secureJson({ error: 'invalid paths' }, { status: 400 });
