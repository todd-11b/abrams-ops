...
beforeEach(() => {
  ...
  process.env.OPERATOR_TODD_PIN = '7419'; process.env.OPERATOR_TY_PIN = '5806';
...
describe('operator session endpoint', () => {
  ...
  it('returns a provider-neutral operator identity for a valid server PIN', async () => {
    const res = await handler(new Request('http://test/api/operator/session', { method: 'POST', headers: { 'x-vercel-forwarded-for': '203.0.113.4' }, body: JSON.stringify({ pin: '5806' }) }));
  ...
  it('fails closed with the same generic response when the durable limiter is locked or unavailable', async () => {
    ...
    const locked = await handler(new Request('http://test/api/operator/session', { method: 'POST', headers: { 'x-vercel-forwarded-for': '203.0.113.4' }, body: JSON.stringify({ pin: '5806' }) }));
  ...
  it('rejects missing or ambiguous trusted address input and ignores spoofable forwarding headers', async () => {
    ...
    const spoofed = await handler(new Request('http://test/api/operator/session', { method: 'POST', headers: { 'x-forwarded-for': '203.0.113.4' }, body: JSON.stringify({ pin: '5806' }) }));
    expect(spoofed.status).toBe(401); expect(fetchMock).not.toHaveBeenCalled();
    const ambiguous = await handler(new Request('http://test/api/operator/session', { method: 'POST', headers: { 'x-vercel-forwarded-for': '203.0.113.4, 198.51.100.2' }, body: JSON.stringify({ pin: '5806' }) }));
