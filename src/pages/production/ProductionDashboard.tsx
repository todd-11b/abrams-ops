...
export default function ProductionDashboard() {
  ...
  const paid = useMemo<Job[]>(() => jobs.filter((j) => j.deposit_status === 'paid'), [jobs]);
  const awaitingDeposit = useMemo<Job[]>(() => jobs.filter((j) => j.deposit_status !== 'paid'), [jobs]);

  const sorted = useMemo<Job[]>(() => {
    const blocked = paid.filter((j) => j.status === 'blocked');
    const rest = paid.filter((j) => j.status !== 'blocked');
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="p-4 md:p-6 space-y-3 max-w-3xl mx-auto">
        ...
        {!loading && sorted.length === 0 && awaitingDeposit.length === 0 && (
        ...
        {awaitingDeposit.length > 0 && (
          <>
            <h2 className="text-xs font-semibold uppercase tracking-wide text-slate-500 pt-4">
              Awaiting deposit
            </h2>
            {awaitingDeposit.map((j) => <JobCard key={j.job_id} job={j} openIssueCount={countsByJob[j.job_id] ?? 0} />)}
          </>
        )}
      </main>
    </div>
