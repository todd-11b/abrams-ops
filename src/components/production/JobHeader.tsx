...
export function JobHeader({ job, spec, address, customerName }: Props) {
  const accessLines = Object.entries(job.access_notes ?? {})
    ...
    .map(([k, v]) => `${k}: ${String(v)}`);
  // If the GHL contact doesn't have a name yet, fall back to the job number as the
  // primary identifier instead of showing an apologetic placeholder string.
  const heading = customerName.trim() || job.job_number;
  return (
